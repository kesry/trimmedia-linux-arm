...
将需要刮削与播放的影视文件放入媒体目录（默认 `BINPATH/data/media`），可通过环境变量 `MEDIA_DIR` 指定。媒体目录下的每个一级子目录都会被识别为一个可授权路径。
...
| `MEDIA_DIR` | `BINPATH/data/media` | 媒体文件目录 |
| `LOG_LEVEL` | `info` | 日志级别 |
...
```bash
WEB_PORT=9000 MEDIA_DIR=/data/movies ./fntv.arm64
```
